// Package server provides HTTP server setup and configuration using Fiber framework.
package server
...
func NewServer(config config.App) *FiberServer {
	...
	app.Get("/docs/index.html", func(ctx *fiber.Ctx) error {
		...
		if _, err := ctx.Response().BodyWriter().Write([]byte(`<!doctype html>
<html>
  <head>
  ...
  <body>
    ...
    <script src="https://cdn.jsdelivr.net/npm/@scalar/api-reference"></script>
			...
			zap.Error(ctx, "Error providing API docs.", err)
		}
		return nil
	})
